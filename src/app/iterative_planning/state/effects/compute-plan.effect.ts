...
import { createIterationStepFailure, createIterationStepSuccess, loadIterationSteps, planComputationRunningFailure, planComputationRunningSuccess, registerPlanComputationSuccess, registerPlanComputation} from "../iterative-planning.actions";
...
export class ComputePlanEffect{
    ...
    computePlanOnIterationStepSuccess$ = createEffect(() => this.actions$.pipe(
      ...
      switchMap(({iterationStep: { _id: iterationStepId }}) => this.plannerService.postComputePlan$(iterationStepId).pipe(
          concatLatestFrom(() => this.store.select(selectIterativePlanningProject)),
          switchMap(([res, project]) => [registerPlanComputationSuccess({iterationStepId}), loadIterationSteps({id: project._id})]),
          catchError((err) => of(createIterationStepFailure()))
      )),
    ));
