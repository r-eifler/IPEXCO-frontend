...
export class QuestionCreatorGuard implements CanDeactivate<QuestionCreatorComponent> {
  ...
  canDeactivate(
    ...
    nextState?: RouterStateSnapshot): boolean | Promise<boolean | UrlTree> {
  ...
  checkQuestionComputed(currentState: RouterStateSnapshot): Promise<boolean | UrlTree> {
    return new Promise<boolean|UrlTree>((resolve, reject) => {
      this.plannerService.isPlannerBusy().subscribe(
        (busy) => {
          if (! busy) {
            combineLatest([this.selectedPlanRunService.getSelectedObject(), this.selectedQuestionService.getSelectedObject()])
              .subscribe(
                ([planRun, expRun]) => {
                  if (planRun && expRun) {
                    const path = currentState.url.replace('new-question', '') + 'question-step/' + expRun._id;
