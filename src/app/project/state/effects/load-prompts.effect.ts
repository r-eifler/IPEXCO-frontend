import { inject, Injectable } from "@angular/core";
import { Actions, createEffect, ofType } from "@ngrx/effects";
import { catchError, switchMap } from "rxjs/operators";
import { of } from "rxjs";
import { ProjectPromptsService } from "../../service/prompts.service";
import { loadOutputSchemas, loadOutputSchemasFailure, loadOutputSchemasSuccess, loadPrompts, loadPromptsFailure, loadPromptsSuccess } from "../project.actions";



@Injectable()
export class ProjectLoadPromptsEffect{

    private actions$ = inject(Actions)
    private service = inject(ProjectPromptsService)

    public loadPrompts$ = createEffect(() => this.actions$.pipe(
        ofType(loadPrompts),
        switchMap(() => this.service.getPrompts$().pipe(
            switchMap(prompts => [loadPromptsSuccess({prompts})] ),
            catchError((e) => of(loadPromptsFailure({err: e})))
        ))
    ))


    public loadOutputSchema$ = createEffect(() => this.actions$.pipe(
        ofType(loadOutputSchemas),
        switchMap(() => this.service.getOutputSchemas$().pipe(
            switchMap(schemas => [loadOutputSchemasSuccess({outputSchemas: schemas})] ),
            catchError((e) => of(loadOutputSchemasFailure({err: e})))
        ))
    ))
}