...
export function predicateToFact(pred: Predicat): Fact {
    return {name: pred.name, arguments: pred.parameters.map(p => p.name), negated: pred.negated};
  }

export function instantiatePrediacteFromArgs(pred: Predicat, args: string): Predicat {
  let init_params = []
...
export function instantiatePredicateFromArgsMap(pred: Predicat, args: Map<string,string>): Predicat {
  return {name: pred.name,
    parameters: pred.parameters.map(p => {return {name: args.get(p.name), type: p.type}}),
    negated: pred.negated};
}

export function instantiatePredicateAll(pred: Predicat, typeObjectMap: Map<string,string[]>): Fact[] {
  ...
  return init_params.map(ps => ({name: pred.name, arguments: ps, negated: pred.negated}));
}
...
export function instantiateFactFromArgsMap(fact: Fact, args: Map<string,string>): Fact {
  let init_args = []
...
export function instantiateAction(action: Action, args: string[]): Action {
    ...
    let i_params: Object[] = action.parameters.map(o => {return {name: args_map.get(o.name), type: o.type}})
    ...
    return {name: action.name, parameters: i_params, precondition: i_precon, effects: i_eff}
}

export function actionToPDDL(action: Action): string {
    let s = "(:action " + action.name + "\n";
    s += "\tparameters: " + action.parameters.map(p => "(" + p.name + ")").join(' ') + "\n";
    s += "\tprecondition: (and " + action.precondition.map(p => factToPDDL(p)).join(' ') + ")\n";
    s += "\teffect: (and " + action.effects.map(p => factToPDDL(p)).join(' ') + ")\n";
    return s + ")\n";
}
...
export function getObjectTypeMap(task: PlanningTask): Map<string, string[]> {
  ...
  otmap.set('object', task.objects.map(o => o.name));
...
export function toPDDLDomain(task: PlanningTask): string {
    ...
    d += "(:types " + task.types.map(t => t.name + "-" + t.parent).join("\n") + "\n)\n";
    d += "(predicates: " + task.predicates.map(p => predicateToPDDL(p,true)).join("\n") + "\n)\n";
...
export function toPDDLProblem(task: PlanningTask): string {
  ...
  p += "(:objects " + task.objects.map(o => o.name + "-" + o.type).join("\n") + "\n)\n";
