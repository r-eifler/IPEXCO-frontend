...
interface ActionSetsTemplates {
  name: string;
  actionTemplates: string[];
}

class ConstraintDomains {
  ...
  addVarValuePosibleValues(name: string, value: string, constrainedVarName: string, possibleValues: Set<string>) {
...
export class PlanPropertyTemplate {
    ...
    public class: string;
    public type: string;
    public variables: {name: string; type: string}[];
    ...
    public goaltVariableConstraints: string[];
        ...
        for (const v of entry1[1]) {
          for (const entry2 of varValues.entries()) {
            if (entry2[0] !== entry1[0]) {
              ...
              if (constrained) {
                this.constraintDomains.addVarValuePosibleValues(constrainigVar.name, constrainigVar.value,
                  constrainedVar.name, constrainedVar.values);
              } else {
                this.constraintDomains.addVarValuePosibleValues(constrainigVar.name, constrainigVar.value,
                  constrainedVar.name, varValues.get(constrainedVar.name));
              }
            } else {
              this.constraintDomains.addVarValuePosibleValues(entry1[0], v, entry1[0], varValues.get(entry1[0]));
            }
          }
        }
      }
    ...
    generatePlanProperty(varValueMapping: Map<string, string>, taskSchema: TaskSchema, project: Project): PlanProperty {
      ...
      return {name, goalType: GoalType.planProperty, type: this.type,  formula,
    ...
    findVarValue(varName: string, varValueMapping: Map<string, string>, taskSchema: TaskSchema): string {
      for ( const constraint of this.goaltVariableConstraints) {
        if (constraint.includes(varName)) {

          let constraintInstance = constraint.replace(varName, '(\\w+)');
          for (const pair of varValueMapping.entries()) {
...
function domainIntersection(domains1: Map<string, Set<string>>, domains2: Map<string, Set<string>>): Map<string, Set<string>> {
...
function getConstraintSatValues(
  ...
  for (const con of constraints) {
    if (con.includes(constrainingVar.name) && con.includes(constrainedVar.name)) {
      ...
      let conRegexString = con.replace(constrainingVar.name, constrainingVar.value);
...
function instanciateContraint(constraint: string, varValueMapping: Map<string, string>): string {
