...
export class PlanPropertyTemplate {
        ...
        for (const entry2 of varValues.entries()) {
          if (entry2[0] !== entry1[0]) {
            ...
            if (constrained) {
              ...
              this.constraintDomains.addVarValuePosibleValues(constrainigVar.name, constrainigVar.value, constrainedVar.name, varValues.get(constrainedVar.name));
            }
          }
          else {
  ...
  generatePlanProperty(varValueMapping: Map<string, string>, taskSchema: TaskSchema, project: Project): PlanProperty {
    ...
    return {
      ...
      actionSets, naturalLanguageDescription, project: project._id, isUsed: false, globalHardGoal: false, value: 1,
    };
  }
...
export function domainIntersection(domains1: Map<string, Set<string>>, domains2: Map<string, Set<string>>): Map<string, Set<string>> {
