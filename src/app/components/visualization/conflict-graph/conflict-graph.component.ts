          ...
          for (var j = 0; j < arrConflict[i].length; j++) {
            ...
            svg.append("rect")
              ...
              .on("mouseover", function (e: any, d) {
                ...
                var newX = Math.round(((this.x.animVal.value + 0.5 * boxSize - margin.left) / 0.5 * arrPlanProperties.length / (width - margin.right - margin.left) - 1) / 2);
                var newY = Math.round(((this.y.animVal.value + 0.5 * boxSize - 1) / 0.5 * dummy.length / (height - margin.bottom - margin.top) - 1) / 2);
                boxtip.html("Goal: " + arrPlanProperties[newX] + "</br>" + "belongs to: " + dummy[newY])
                  .style("left", (e.clientX - 480) + "px")
                  .style("top", function () {
