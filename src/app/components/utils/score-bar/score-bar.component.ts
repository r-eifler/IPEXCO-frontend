...
export class ScoreBarComponent implements OnInit, OnDestroy {
  ...
  constructor() {
    ...
    this.left$ = combineLatest([this.min$, this.max$, this.v_small$, this.v_large$]).pipe(
      takeUntil(this.unsubscribe$),
      filter(([min, max, v_small, v_large]) => min != null && max != null && v_small != null && v_large != null),
    ...
    this.width$ = combineLatest([this.min$, this.max$, this.v_small$, this.v_large$]).pipe(
      takeUntil(this.unsubscribe$),
      filter(([min, max, v_small, v_large]) => min != null && max != null && v_small != null && v_large != null),
      map(([min, max, v_small, v_large]) => {
        let div = max - min;
