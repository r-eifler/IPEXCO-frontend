...
export class TaskViewComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    combineLatest([
      this.currentRunStore.item$,
      planPropertyCollectionService.getMap(),
    ])
      ...
      .subscribe(([run, planProperties]) => {
        ...
        if (run && planProperties) {
          ...
          // }
          this.enforcedSatPlanProperties.sort((a, b) =>
            a.globalHardGoal ? -1 : 0
          );
          ...
          if (this.hasPlan) {
            ...
            for (const prop of planProperties.values()) {
              if (
                !this.enforcedSatPlanProperties.find(
                  (p) => p.name === prop.name
                ) &&
