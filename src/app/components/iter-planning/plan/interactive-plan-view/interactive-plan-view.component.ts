...
import { Action, Fact, factEquals, FactToString, instantiateAction } from 'src/app/interface/plannig-task';
      ...
      ([step, plan]) => {
        if(step && plan){
          ...
          for(const action of plan.actions){
            ...
            const i_action = instantiateAction(action_map.get(action.name), action.parameters.map(o => o.name));
            i_action.effects.forEach(item => used_predicate_names.add(item.name))
            this.action_trace.push(i_action);

            let newState = nextState(this.state_trace[this.state_trace.length - 1], i_action);
