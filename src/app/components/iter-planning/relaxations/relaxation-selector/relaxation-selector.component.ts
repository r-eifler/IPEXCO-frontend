...
export class RelaxationSelectorComponent implements OnInit, OnDestroy {
  ...
  newUpdateSelected(
    newSelected: MetaFact,
    oldSelected: MetaFact,
    orgFact: MetaFact
  ): void {
    console.log(orgFact);
    ...
    this.step$
      ...
      .subscribe((step) => {
        step.task.initUpdates = step.task.initUpdates.filter(
          (mt) => !factEquals(mt.orgFact, orgFact.fact)
        );
        if (!factEquals(newSelected.fact, orgFact.fact)) {
          step.task.initUpdates.push({
            orgFact: orgFact.fact,
            newFact: newSelected.fact,
          });
        }
