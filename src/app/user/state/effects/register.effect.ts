...
import { of } from "rxjs";
import { login, loginFailure, loginSuccess, registerUser, registerUserFailure, registerUserSuccess } from "../user.actions";
...
export class RegisterEffect{
    ...
    public loginUser$ = createEffect(() => this.actions$.pipe(
        ...
        switchMap(({name, password}) => this.service.register(name, password).pipe(
            switchMap(({user, token}) => [registerUserSuccess({user, token})]),
