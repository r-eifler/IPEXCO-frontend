...
export const selectPlanPropertiesListOfDemo = createSelector(demosFeature.selectDemosFeatureState, 
    (state) => state.demo?.data?._id ? state.demoProperties[state.demo?.data?._id] : undefined);
export const selectPlanPropertiesOfDemo = createSelector(demosFeature.selectDemosFeatureState, 
    (state) => state.demo?.data?._id ? state.demoProperties[state.demo?.data?._id]?.reduce((acc, cv) => ({...acc,[cv._id]: cv}), {}) : undefined);
