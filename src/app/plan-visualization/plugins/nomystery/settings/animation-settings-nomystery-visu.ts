...
export class AnimationSettingsNoMysteryVisu {
  ...
  constructor(
    ...
    projectService.getSelectedObject().subscribe((project) => {
      if (project) {
        ...
        this.locationPosSettings = new LocationPositioningSettings(
          backgroundImagePath,
          task,
  ...
  save() {
    this.animationSettings.locationPositions =
      this.locationPosSettings.getCurrentLocationsPositions();
    this.animationSettings.locationDropPositions =
      this.locationPosSettings.getCurrentLocationsDropPositions();
