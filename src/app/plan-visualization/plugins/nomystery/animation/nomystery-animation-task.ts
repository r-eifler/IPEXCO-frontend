...
export class AnimationTruck extends AnimationNode {
  ...
  updateFuelDisplay() {
    this.fuelDisplay.innerText =
      this.displayName + " fuel: " + this.currentFuel;
  }
  ...
  animateDriveTo(
    targetLocation: AnimationLocation,
    ...
    return new Promise((resolve, reject) => {
      ...
      const tween = gsap.to(this.group, {
        duration: 0.5,
        x: this.x,
        y: this.y,
        ease: "power4. out",
...
export class AnimationPackage extends AnimationNode {
  ...
  constructor(
    ...
    public startLocation: AnimationLocation,
    public goalLocation: AnimationLocation
  ) {
    super(pack.name);
  }
  ...
  animateLoad(truck: AnimationTruck): Promise<void> {
    ...
    const targetPosition = {
      x: truck.x + relTargetPosition.x,
      y: truck.y + relTargetPosition.y,
    };
    this.x = targetPosition.x;
    this.y = targetPosition.y;

    return new Promise((resolve, reject) => {
      const tween = gsap.to(this.svg, {
        duration: 1,
        x: this.x,
        y: this.y,
        ease: "power4. out",
      ...
      tween.then(() => {
        ...
        gsap.to(this.svg, {
          duration: 0,
          x: relTargetPosition.x,
          y: relTargetPosition.y,
          ease: "power4. out",
        });
        resolve();
      });
...
export class NoMysteryAnimationTask extends AnimationInfo {
  ...
  constructor(
    protected task: NoMysteryTask,
    private dropPositions: Map<string, Position[]>
  ) {
    super(task);
    this.buildFromTask();
  }
  ...
  buildFromTask(): void {
    ...
    for (const truck of this.task.trucks.values()) {
      this.trucks.set(
        truck.name,
        new AnimationTruck(truck, this.locations.get(truck.startLocation.name))
      );
    }
    for (const pack of this.task.packages.values()) {
      this.packages.set(
        ...
        new AnimationPackage(
          pack,
          this.locations.get(pack.startLocation.name),
          this.locations.get(pack.goalLocation.name)
        )
      );
    }
  }
