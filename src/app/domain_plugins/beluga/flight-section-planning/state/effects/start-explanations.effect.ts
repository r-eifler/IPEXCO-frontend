...
import { explanationsFinishedFailure, explanationsFinishedSuccess, loadFlightsHorizons, reloadFlightPlanTree, startExplanations, startExplanationsFailure, startExplanationsSuccess } from "../flight-section-planning.actions";
...
export class StartExplanationEffect{
    ...
    private service = inject(FlightsHorizonExplanationService);
    ...
    public start$ = createEffect(() => this.actions$.pipe(
        ...
        switchMap(({section, configIndex}) => this.service.postExplanationRequest$(section, configIndex).pipe(
            switchMap(section => [startExplanationsSuccess({section, configIndex}), loadFlightsHorizons({treeId: section.treeId})]),
