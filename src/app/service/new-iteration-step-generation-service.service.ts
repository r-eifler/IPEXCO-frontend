...
import { NewIterationStepStoreService, SelectedIterationStepService } from './planner-runs/selected-iteration-step.service';
...
export class NewIterationStepGenerationService {
  ...
  finishNewStep(): void {
    ...
    pipe(
      filter(([step, planProperties, project]) => !!step && !!planProperties && !!project),
      take(1)).subscribe(
...
export class DemoNewIterationStepGenerationService extends NewIterationStepGenerationService {
  ...
  constructor(
    ...
    super(newIterationStepStoreService, iterationStepsService, selectedIterationStepService, planPropertyMapService, plannerService, currentProjectService);
  ...
  createInitialStep() {
    ...
    combineLatest([this.demo$, this.planProperties$, this.relaxationSpaces$]).
      pipe(
        filter(([demo, ppM, spaces]) => !!demo && !!ppM && ppM.size > 0 && !!spaces && spaces.length > 0),
        take(1)).subscribe(
        ([demo, planProperties, relaxationSpaces]) => {
          console.log("Generate initial Iteration Step");
          console.log("#planProperties: " + planProperties.size);
          let softGoals = [];
          let hardGoals = [];
          ...
          if(demo.settings.computeDependenciesAutomatically){
            solvable = this.plannerService.computeRelaxExplanations(newStep, demo);
          }
    ...
    finishNewStep(): void {
      ...
      pipe(
        filter(([step, planProperties, demo]) => !!step && !!planProperties && !!demo),
        take(1)).subscribe(
        ([step, planProperties, demo]) => {
          if (step && planProperties) {
            // let name = 'Step ' +  this.iterationStepsService.getNumRuns();
            let softGoals = [];
            for (const pp of planProperties.values()) {
